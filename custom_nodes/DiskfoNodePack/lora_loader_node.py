...
def _is_lora_wrapper(mod: nn.Module) -> bool:
    return isinstance(mod, LoRALinearWrapper)
...
class ACEStepLoRALoader:
    ...
    def IS_CHANGED(cls, **kwargs):
        # Force re-execution every prompt to avoid cached outputs
        import time
        return time.time()
    @classmethod        
    def INPUT_TYPES(cls):
